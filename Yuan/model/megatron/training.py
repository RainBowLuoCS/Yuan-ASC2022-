...
def print_datetime(string,file=None):
        ...
        if torch.distributed.is_initialized():
            if torch.distributed.get_rank() == 0:
                with open(file,'a') as f:
                    f.write('[' + string + '] datetime: {} '.format(time_str))
...
def pretrain(train_valid_test_dataset_provider,
    ...
    print_datetime('after training is done',args.log_path)
...
def train(forward_step_func, model, optimizer, lr_scheduler,
    ...
    print_datetime('before the start of training step',args.log_path)
    f=None
    if is_last_rank():
        if args.log_path_detail:
            f=open(args.log_path_detail,'a')
